...
int main(int argc, char** argv) {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr stool_cloud_ptr(new pcl::PointCloud<pcl::PointXYZ>); //ptr to selected pts from Rvis tool
    pcl::PointCloud<pcl::PointXYZ>::Ptr coke_cloud_ptr(new pcl::PointCloud<pcl::PointXYZ>); //ptr to selected pts from Rvis tool
    ...
    if (pcl::io::loadPCDFile (find_in_package("coke_top.pcd"), *coke_cloud_ptr) < 0)
    {
        std::cout << "Error loading coke_top cloud." << std::endl;
        return (-1);
    }
    if (pcl::io::loadPCDFile (find_in_package("stool.pcd"), *stool_cloud_ptr) < 0)
    {
        std::cout << "Error loading stool cloud." << std::endl;
        return (-1);
    }
    ...
    while (ros::ok()) {
        if (jump++ % 200 == 0) {
            ROS_WARN("Finding stool:");
            find_indices_of_plane_from_patch(downsampled_kinect_ptr, stool_cloud_ptr, transformed_cloud_ptr, indices);
            sphare_filter(downsampled_kinect_ptr, stool_cloud_ptr, indices, filterd_indices, center_indices, 0.195);
            find_normal_centroid(downsampled_kinect_ptr, filterd_indices,  plane_normal, plane_centroid, plane_distance);
        }
        if ((jump+100) % 200 == 0) {
            ROS_WARN("Finding coke top:");
            find_indices_of_plane_from_patch(downsampled_kinect_ptr, coke_cloud_ptr, transformed_cloud_ptr, indices);
            sphare_filter(downsampled_kinect_ptr, coke_cloud_ptr, indices, filterd_indices, center_indices, 0.1);
            find_normal_centroid(downsampled_kinect_ptr, filterd_indices,  plane_normal, plane_centroid, plane_distance);
        }
