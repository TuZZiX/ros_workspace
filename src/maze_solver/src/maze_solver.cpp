  ...
  void move(int direction, double time) {
    ...
    switch (direction) {
      ...
      case BACKWARD:
        ...
        break;
      case LEFT:
        twist_cmd.linear.z = speed;
        break;
      case RIGHT:
        twist_cmd.linear.z = -1.0 * speed;
        break;
      default:
        break;
    }
...
int main(int argc, char **argv) {
  ...
  robot.move(FORWARD, 3.0);
  robot.turn(RIGHT);
  robot.move(FORWARD, 4.0);
  robot.turn(LEFT);
  robot.move(FORWARD, 2.2);
  robot.turn(LEFT);
  robot.move(FORWARD, 4.2);
  robot.turn(RIGHT);
  robot.move(FORWARD, 6.8);
  robot.turn(LEFT);
  robot.move(FORWARD, 2.8);
