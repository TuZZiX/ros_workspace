...
#define CONTROL_SPEED 100			/* this represent how fast this program runs, faster control speed reduce response time and smooth the output trajectory, slow down control spped can save your CPU resource*/
...
int main( int argc, char **argv )
{
	...
	double  jnt_cmd;					// joint desired position
	...
	while ( ros::ok() )
	{
		...
		jnt_cmd = jnt1_amp * (std::sin( ( (M_PI * 2) / (CONTROL_SPEED / jnt1_feq) ) * jnt1_cnt ) );	// calculate value of single point in the wave
		
		get_jnt_state_client.call( get_joint_state_srv_msg );			// get joint1 state
		jnt_pos		= get_joint_state_srv_msg.response.position[0];
		jnt_vel		= get_joint_state_srv_msg.response.rate[0];
		
		joint_state_msg.header.stamp	= ros::Time::now();				// fill the message
		joint_state_msg.position[0]	= jnt_pos;
		joint_state_msg.velocity[0]	= jnt_vel
		joint_state_publisher.publish( joint_state_msg );				// publish this state to robot_state_publisher
		
		error = jnt_cmd - jnt_pos;			// calculate control error
		...
		effort_cmd_srv_msg.request.effort = trq_cmd;	// tell gazebo our new control singal
		set_trq_client.call( effort_cmd_srv_msg );		// send to gazebo
		...
		if (jnt2_cnt++ >= (CONTROL_SPEED / jnt2_feq))
			jnt2_cnt = 0;
		jnt_cmd = jnt2_amp * (std::sin( ( (M_PI * 2) / (CONTROL_SPEED / jnt2_feq) ) * jnt2_cnt ) );
		
		get_jnt_state_client.call( get_joint_state_srv_msg2 );
		jnt_pos		= get_joint_state_srv_msg2.response.position[0];
		jnt_vel		= get_joint_state_srv_msg2.response.rate[0];
		joint_state_msg2.header.stamp	= ros::Time::now();
		joint_state_msg2.position[0]	= jnt_pos;
		joint_state_msg2.velocity[0]	= jnt_vel
		
		joint_state_publisher.publish( joint_state_msg2 );
		error = jnt_cmd - jnt_pos;
		if ( error > M_PI )
		{
			error -= 2 * M_PI;
		}
