...
double	amp	= 1.0;          /* set a default value to amplitude */
double	feq	= 0.5;          /* set a default value to frequency */
...
int main( int argc, char **argv )
{
	...
	while ( ros::ok() )
	{
		for ( int count = 0; count < (CONTROL_SPEED / feq); count++ )
		{
			g_pos_cmd = amp * (std::sin( ( (M_PI * 2) / (CONTROL_SPEED / feq) ) * count ) );
